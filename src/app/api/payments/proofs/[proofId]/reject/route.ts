...
import { rejectManualPaymentProof } from "@/modules/payments/payment-ops.service";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ proofId: string }> }) {
  ...
  try {
    const detail = await rejectManualPaymentProof({
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to reject manual payment proof" },
      { status: 400 },
    );
  }
}
