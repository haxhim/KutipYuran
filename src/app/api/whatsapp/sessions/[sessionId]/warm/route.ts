...
import { warmWhatsappSession } from "@/modules/whatsapp/whatsapp.service";
...
export async function POST(_: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const result = await warmWhatsappSession(session.sessionKey, session.id, tenant.organizationId);
